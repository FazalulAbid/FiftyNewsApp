...
import com.fifty.fiftynews.preference.AuthPreference
import com.fifty.fiftynews.util.Constants
import com.fifty.fiftynews.util.Constants.Companion.PROFILE_IMAGES
import com.fifty.fiftynews.util.Constants.Companion.UID
...
class UserRepository(
    ...
    suspend fun saveUserProfileData(user: User, result: (UiState<Boolean>) -> Unit) =
                ...
                docRef.get().addOnCompleteListener {
                    if (it.isSuccessful) {
                        ...
                        if (document != null && document.exists()) {
                            ...
                            if (continuation.isActive) continuation.resume(result.invoke(UiState.Success(false)))
                        }
                    } else {
                        if (continuation.isActive) continuation.resume(result.invoke(UiState.Success(false)))
                    }
